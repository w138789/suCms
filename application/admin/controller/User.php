...
class User extends Admin {
    ...
    public function add() {
        ...
        if (IS_POST)
        {
            ...
            try
            {
                ...
                //创建注册用户
                $result = $model->register($data['username'], $data['password'], $data['email'], false);
                if ($result)
                {
    ...
    public function del($id) {
        $id = array('IN', is_array($id) ? implode(',', $id) : $id);
        //获取用户信息
        $find = $this->getUserinfo($id);
        model('User')->where(array('id' => $id))->delete();
    ...
    public function auth() {
        ...
        if (IS_POST) {
            ...
            $uid  = input('uid', '', 'trim,intval');
            $row  = $group::where(['module'=>'front'])->select();
     ...
     * @author colin <colin@tensent.cn>
     */
    private function getUserinfo($id = null, $pass = null, $errormsg = null) {
        $user = model('User');
        $id  = $id ? $id : input('id');
        //如果无id则修改当前用户
        //$id        = $id ? $id : session('admin_auth.admin_id');
        $map['id'] = $id;
        if ($pass != null) {
            unset($map);
            $map['password'] = $pass;
        }
        $list = $user::where($map)->field('id,username,nickname,sex,email,qq,score,signature,status,salt')->find();
        if (!$list) {
    ...
    public function submitNickname() {
        ...
        $res = $Member->where(array('id' => $id))->save($data);
