...
function get_cover($cover_id, $field = null) {
    ...
    if ($field == 'path') {
        if (!empty($picture['url'])) {
            $picture['path'] = $picture['url'] ? $picture['url'] : '/images/default.png';
        } else {
            $picture['path'] = $picture['path'] ? $picture['path'] : '/images/default.png';
        }
    }
...
function get_username($id = 0) {
    ...
    $name = db('member')->where(array('id' => $id))->value('username');
    return $name;
}
...
function avatar($uid, $size = 'middle') {
    ...
    $file = '/uploads/avatar/' . $dir . 'avatar_' . $size . '.png';
    if (!file_exists('.' . $file)) {
        $file = '/images/default_avatar_' . $size . '.jpg';
    }
    return $file;
}
...
function get_picture($id){
    return model('Picture')->where(['id'=>$id])->find();
}
