...
class Admin extends Base {
    ...
    {
        $admin = $this->admin->get(['username' => $username]);
        if (empty($admin))
        {
            //账号为空
            return -2;
    ...
    public function autoLogin($admin)
    {
        ...
        $this->admin->save($data, ['admin' => $admin['admin_id']]);
    ...
    final protected function checkRule($rule, $type = AuthRule::rule_url, $mode = 'url') {
        ...
        if (!$Auth->check($rule, session('admin_auth.admin_id'), $type, $mode)) {
            return false;
        }
        return true;
    }
    ...
    protected function setMenu() {
        $hover_url  = $this->request->module() . '/' . parse_name($this->request->controller(),0);
        $controller = $this->url;
        $menu       = array(
            'main'  => array(),
            'child' => array(),
    ...
    protected function getContentMenu() {
        ...
        //判断是否有模型权限
        $models = AuthGroup::getAuthModels(session('admin_auth.admin_id'));
